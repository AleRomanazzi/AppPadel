...
export const isValidSetScore = (score) => {
    if (!score?.trim())
        return false;
...
export const parseScoreForPairA = (score) => {
...
export const scoreDiffForPair = (score, isPairA) => {
 ...
 * - evita "63-4", "6-77", etc.
 */
export const formatScoreTyping = (previous, nextRaw) => {
    if (nextRaw === "")
        return "";
    ...
    if (previous && nextRaw.length < previous.length) {
        ...
        if (only === "")
            return "";
        if (SCORE_RE.test(only) && isValidSetScore(only))
            return only;
    ...
    if (second > 7)
        return previous.endsWith("-") ? previous : `${first}-`;
